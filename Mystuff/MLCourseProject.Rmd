...
```{r ,cache=TRUE, include=TRUE, echo=TRUE}
rpartModel <- train(classe ~ . , method = "rpart", data=training.data)
...
Lookng at the results from the 3 models, the Random Forest appears to deliver the best results, though perfect accuracy suggests there may be overfitting. The validation dataset will now be used to test accuracy of the model.

## Predicting Validation Classe

The rfModel used earlier will be used to predict the validation classe. The accuracy will then be tested
...
>>>>>>> f397180696a971e0f2e9f2cd6439fffdb5aec6c1
